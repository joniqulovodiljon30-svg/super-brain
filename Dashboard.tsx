  ...
  return (
    <div className="space-y-8">
      <header>
        ...
        <p className="text-slate-500 dark:text-slate-400 mt-2 text-lg">Your memory palace stats at a glance.</p>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="glass rounded-3xl p-6 md:p-8 space-y-6">
          <h3 className="text-xl font-bold">Quick Training</h3>
          <div className="space-y-3">
            <QuickLink label="Word Sequences" desc="Memorize lists (Up to 500)" onClick={() => onNavigate(ModuleType.WORDS)} />
            <QuickLink label="Speed Numbers" desc="Rapid recall (Up to 500)" onClick={() => onNavigate(ModuleType.NUMBERS)} />
            <QuickLink label="Image Grid" desc="Spatial Recall" onClick={() => onNavigate(ModuleType.IMAGES)} />
            <QuickLink label="Flashcards" desc="Review Decks" onClick={() => onNavigate(ModuleType.FLASHCARDS)} />
          </div>
        </div>
      </div>
    </div>
