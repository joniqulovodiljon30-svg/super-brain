...
import { Plus, Trash2, Download, CreditCard, RotateCw } from 'lucide-react';
      ...
      <header className="flex justify-between items-center mb-8">
        <div>
          <h2 className="text-3xl font-bold flex items-center gap-3">
            <CreditCard className="text-indigo-500" /> Flashcards
          </h2>
          <p className="text-slate-500">Spaced repetition for long-term memory.</p>
        </div>
        {!studyMode && (
      ...
      {!studyMode ? (
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            {cards.map(card => (
              <div key={card.id} className="h-48 glass rounded-3xl p-6 relative group border border-white/5 overflow-hidden">
                <button 
                ...
                <div className="text-[10px] uppercase font-bold text-indigo-500 mb-2">{card.category}</div>
                <h4 className="font-bold text-md mb-2 line-clamp-3">{card.front}</h4>
                <div className="mt-auto pt-4 border-t border-slate-100 dark:border-slate-800 absolute bottom-6 left-6 right-6">
        ...
        <div className="max-w-xl mx-auto space-y-8 animate-in zoom-in-95 duration-300">
          ...
          >
            <div className={`w-full h-full duration-500 relative ${isFlipped ? '[transform:rotateY(180deg)]' : ''}`} style={{ transformStyle: 'preserve-3d' }}>
              <div className="absolute inset-0 backface-hidden glass rounded-3xl p-8 flex items-center justify-center text-center" style={{ backfaceVisibility: 'hidden' }}>
                <p className="text-2xl font-bold">{cards[currentIndex].front}</p>
                <p className="absolute bottom-6 text-[10px] text-slate-400 flex items-center gap-1 uppercase tracking-widest font-bold"><RotateCw size={12} /> Tap to flip</p>
              </div>
              <div className="absolute inset-0 backface-hidden [transform:rotateY(180deg)] bg-indigo-500 text-white rounded-3xl p-8 flex items-center justify-center text-center" style={{ backfaceVisibility: 'hidden' }}>
          ...
          <button onClick={() => setStudyMode(false)} className="w-full text-center text-slate-500 font-bold uppercase tracking-widest text-xs hover:text-indigo-500">
            End Session
          </button>
        </div>
      )}
...
const ScoreBtn = ({label, color, onClick}: {label: string, color: string, onClick: () => void}) => (
  <button onClick={onClick} className={`${color} text-white py-3 rounded-xl text-[10px] font-bold uppercase tracking-wider hover:brightness-110 transition-all`}>
    {label}
  </button>
);
