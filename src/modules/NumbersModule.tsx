...
import { Timer, RefreshCw, Zap, CheckCircle2, XCircle, LayoutGrid } from 'lucide-react';
        ...
        <div className="glass rounded-3xl p-6 md:p-10 space-y-8 animate-in fade-in duration-300">
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <button
              ...
              className="flex-1 py-5 bg-indigo-500 hover:bg-indigo-600 text-white rounded-2xl font-bold text-xl shadow-xl shadow-indigo-500/30 transition-all hover:scale-[1.01] active:scale-95"
            >
              Start Memorization
            </button>
            <button
              onClick={() => setShowMajorSystemEditor(true)}
              className="px-8 py-5 bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-700 dark:text-slate-200 rounded-2xl font-bold text-xl transition-all flex items-center justify-center gap-3"
            >
              <LayoutGrid size={24} /> O'rganish
            </button>
          </div>
          {showMajorSystemEditor && <MajorSystemEditor onClose={() => setShowMajorSystemEditor(false)} />}
        </div>
      )}
